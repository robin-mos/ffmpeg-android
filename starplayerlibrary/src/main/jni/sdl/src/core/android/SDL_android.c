...
JNIEXPORT int JNICALL SDL_JAVA_INTERFACE(nativeRunMain)(JNIEnv* env, jclass cls, jstring library, jstring function, jobject array)
{
    ...
    __android_log_print(ANDROID_LOG_ERROR, "SDL", "nativeRunMain()");
    ...
    if (library_handle) {
		...
		__android_log_print(ANDROID_LOG_ERROR, "SDL", "array:%s",array);

            /* Prepare the arguments. */
            len = (*env)->GetArrayLength(env, array);
            argv = SDL_stack_alloc(char*, 1 + len + 1);
		__android_log_print(ANDROID_LOG_ERROR, "SDL", "array len:%d",len);
            argc = 0;
            /* Use the name "app_process" so PHYSFS_platformCalcBaseDir() works.
               https://bitbucket.org/MartinFelis/love-android-sdl2/issue/23/release-build-crash-on-start
             */
            argv[argc++] = SDL_strdup("app_process");
			__android_log_print(ANDROID_LOG_ERROR, "SDL", "SDL_strdup");
            for (i = 0; i < len; ++i) {
                const char* utf;
                char* arg = NULL;
          ...
          __android_log_print(ANDROID_LOG_ERROR, "SDL", "main");
	    //status = android_main(argc,argv);
            ...
            SDL_stack_free(argv);
			__android_log_print(ANDROID_LOG_ERROR, "SDL", "free");
