    ...
    if (is->subtitle_st) {
        if (frame_queue_nb_remaining(&is->subpq) > 0) {
            ...
            if (vp->pts >= sp->pts + ((float) sp->sub.start_display_time / 1000)) {
			   ...
			   LOGE("image w:%d,h:%d",sub_rect->w,sub_rect->h);
...
static void event_loop(VideoState *cur_stream)
{
    ...
    for (;;) {
	 ...
	 LOGE("event type:0x%x\n",event.type);
        switch (event.type) {
        case SDL_KEYDOWN:
	     LOGE("scancode:0x%x,keysym.sym=0x%x,  exit_on_keydown:0x%x\n",event.key.keysym.scancode,event.key.keysym.sym,exit_on_keydown);
            if (exit_on_keydown) {
                do_exit(cur_stream);
                break;
            }
...
int android_main(int argc, char **argv)
{
	...
	LOGE("android main enter argc:%d",argc);
    init_dynload();
    ...
    SDL_GetDisplayUsableBounds(0,&mRect);
	LOGE("screen x:%d,y:%d,w:%d,h:%d",mRect.x,mRect.y,mRect.w,mRect.h);
