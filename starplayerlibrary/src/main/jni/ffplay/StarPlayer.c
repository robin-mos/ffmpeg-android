...
JNIEXPORT jint JNICALL Java_com_robin_starplayer_media_StarPlayerSystem_onStop
  ...
  JNIEXPORT jint JNICALL Java_com_robin_starplayer_media_StarPlayerSystem_prepare(JNIEnv *env, jclass jclass,jobject surface){
	...
	LOGE("avformat_network_init ret =%d",ret);
	AVFormatContext *pFormatContext = avformat_alloc_context();

    LOGE("step avformat_open_input");
      //open video file
       ret = avformat_open_input(&pFormatContext,path,0,0);
...
JavaVM* mJavaVM;
jobject mVideoObject;

/*回调时间接口
* add by robin
*/
 void timeStamp(int hour,int min,int second,int duration){
 	JNIEnv *env;
	
	(*mJavaVM)->AttachCurrentThread(mJavaVM,&env,NULL);
	jclass jclazz= (*env)->GetObjectClass(env,mVideoObject);
	jmethodID id = (*env)->GetMethodID(env,jclazz,"timeStamp","(IIII)V");
	(*env)->CallVoidMethod(env,mVideoObject,id,hour,min,second,duration);
 }


 JNIEXPORT void JNICALL Java_com_robin_starplayer_media_StarPlayerSystem_ffplayProgress(JNIEnv *env, jclass jclzz,jobject obj){
	(*env)->GetJavaVM(env,&mJavaVM);
	mVideoObject = (*env)->NewGlobalRef(env,obj);
	mTimeStampCallBack = timeStamp;
 }
